...
/* static */ auto RhoTrader::getLocalTime(bool underscore) -> std::string
{
...
RhoTrader::RhoTrader(std::string username, std::initializer_list<std::string> symbols /* = {} */)
...
RhoTrader::RhoTrader(std::string username, std::string wealthTrackingFileName, std::initializer_list<std::string> symbols /* = {} */)
    : CoreClient { std::move(username) }
    , m_wealthTrackingFile { std::move(wealthTrackingFileName) }
    , m_symbols { std::move(symbols) }
{
    if (m_wealthTrackingFile) {
        m_wealthTrackingFile << std::fixed << std::setprecision(2); // Price should always have two decimal places
        m_wealthTrackingFile << "real_time,asset,shares,price" << std::endl;
    }
}

void RhoTrader::receiveLastPrice(const std::string& symbol)
{
    // if (isVerbose()) {
    //     if (std::find(m_symbols.begin(), m_symbols.end(), symbol) != m_symbols.end()) {
    //         std::cout << symbol << " Trade: " << std::put_money(getLastPrice(symbol) * 100.0) << std::endl;
    //     }
    // }
}

void RhoTrader::receiveExecution(const std::string& orderID)
{
    if (isVerbose()) {
        ...
        std::cout << order.getSymbol() << ' ' << order.getTypeString() << ' ' << std::put_money(price * 100.0) << ' ' << order.getSize() << ' '
                  << order.getExecutedSize() << ' ' << order.getID() << ' ' << order.getStatusString() << ' ' << std::endl;
    }
}

void RhoTrader::receivePortfolioSummary()
{
    if (isVerbose()) {
        std::cout << "Buying Power: " << std::put_money(getPortfolioSummary().getTotalBP() * 100.0) << std::endl;
    }
    if (m_wealthTrackingFile) {
...
void RhoTrader::receivePortfolioItem(const std::string& symbol)
{
    if (isVerbose()) {
    ...
    if (m_wealthTrackingFile) {
        m_wealthTrackingFile << getLocalTime() << ',';
        m_wealthTrackingFile << symbol << ',' << getPortfolioItem(symbol).getShares() << ',' << getPortfolioItem(symbol).getPrice() << std::endl;
    }
}

void RhoTrader::receiveWaitingList()
{
    // if (isVerbose()) {
    ...
    //             std::cout << order.getSymbol() << ' ' << order.getTypeString() << ' ' << std::put_money(order.getPrice() * 100.0) << ' ' << order.getSize() << ' '
    //                       << order.getExecutedSize() << ' ' << order.getID() << ' ' << order.getStatusString() << ' ' << std::endl;
    //         }
    //     } else {
