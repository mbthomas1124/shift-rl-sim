...
def main(agent_num):#argv: List[str]
    stock_ticker = "CS1"  # stock ticker (e.g. XYZ)
    simulation_duration = 15  # duration of simulation (in minutes)
    ...
    agent_num = agent_num       ########the number of the agent
    ...
    trader = shift.Trader(f"agent{agent_num}")
    trader_list = []
    #for i in range(5): trader_list.append(shift.Trader(f"agent0{agent_num+1+i}"))
    ...
    try:
        ...
        #for i in range(5):trader_list[i].connect("initiator.cfg", "password")
    ...
    for i in range(1, num_trades):  # trading_times[0] == 0
        ...
        if numpy.random.binomial(n=1, p=0.5) == 0:  # limit buy
            ...
            market_buy = shift.Order(
                shift.Order.Type.MARKET_BUY, stock_ticker, 1, order_price
            )
            ...
            market_sell = shift.Order(
                shift.Order.Type.MARKET_SELL, stock_ticker, 1, order_price
            )
...
if __name__ == "__main__":
    #sys.argv[1:]
    num_agents = 200#number of traders (INPUT)
    trader_list = []
    threads = []
    for i in range(0, num_agents+0):  #######run the simulation succesfully, gotta generate some ZI traders with temp_initial.sh or whatever you have on your vm.
                                        #######I changed the number ZI traders in batch.sh to 30.
        ...
        if (i+1) < 10:
            ...
            arg = "0" + str(i+1)
        threads.append(Thread(target=main,args =(arg,)))
