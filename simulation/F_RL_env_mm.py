...
class SHIFT_env(gym.Env):
    ...
    def __init__(
        ...
        self.table = CirList(
            nTimeStep
        )  # contains: 'curr_mp', 'volume_ask', 'volume_bid', 'remained_bp'
        self._cols = [
            "reward",
            "action_size",
            "action_sym",
        ...
        # now it's % instead of action order size
        self.action_space = gym.spaces.Box(
            low=np.array([0.000005, -1, -1]), high=np.array([0.0001, 1, 1]), shape=(3,)
        )  # gym.spaces.Box(low = np.array([1]), high = np.array([5])) #dtype=np.float32,
    ...
    def _link(self):
        ...
        while self.trader.is_connected() and self.thread_alive:
            ...
            Ask_ls = self.trader.get_order_book(
                self.symbol, shift.OrderBookType.LOCAL_ASK, self.ODBK_range
            )
            # assert Ask_ls, f'getOrderBook: return empty list: {self.symbol}-ASK-{self.ODBK_range}'

            Bid_ls = self.trader.get_order_book(
                self.symbol, shift.OrderBookType.LOCAL_BID, self.ODBK_range
            )
            ...
            self.mutex.acquire()
            # print(88)
            self.table.insertData(
                info
            )  # table info: [mid, ask_size, bid_size, bp, inventory]
            # print(tmp)
            self.mutex.release()
    ...
    def LOB_to_list(
        self, ask_book, bid_book, best_p, bp, item
    ):  # return a list with these info 'curr_mp', 'ask_book', 'bid_book', 'remained_bp', inventory, spread
        # get LOB
        bid_b = []
        ask_b = []
        ...
        if (
            abs(mid - self.last_available_price) >= 10
            or mid == 0
            or min(best_p.get_ask_price(), best_p.get_bid_price()) == 0
        ):
            mid = max(
                self.last_available_price,
                max(best_p.get_ask_price(), best_p.get_bid_price()),
            )  # self.table.getData()[self.nTimeStep-1][0]
    ...
    def compute_state_info(self):
        ...
        big_to_small = int(
            self.rl_t / self.info_step
        )  # since 5 * 0.4 = 2 which is the time for each step
        tab = self.table
        while True:
            if tab.isFull():
                his_mp = []
                ...
                price_change = (
                    his_mp[self.nTimeStep - 1] - his_mp[self.nTimeStep - big_to_small]
                )
                # get the inventory pnl
                current_inventory_pnl = (
                    price_change * his_inventory[self.nTimeStep - big_to_small]
                )  # since 5 * 0.4 = 2 which is the time for each step

                book = np.asarray(
                    tab.getData()[self.nTimeStep - 1][1]
                    + tab.getData()[self.nTimeStep - 1][2]
                )
                total_volume = np.sum(book)
                # mean_inven_pnl = np.mean(his_inventory_pnl_np)
                # std_inven_pnl = abs(np.std(his_inventory_pnl_np))
                return (
                    tab.getData()[self.nTimeStep - 1][0:6]
                    + [his_mp[5 : self.nTimeStep]]
                    + [current_inventory_pnl, total_volume]
                )
            else:
        ...
        if actions is None:
            ...
            order_size = int(
                round(
                    ...
                    * actions[0]
                    / (100 * self.last_available_price),
                    0,
                )
            )  # order size#self.trader.get_last_price(self.symbol)
            self.generate_mm_trader(
                self.order_list,
                self.trader,
                self.symbol,
                order_size,
    ...
    def generate_mm_trader(
        ...
        imbalance = int(
            item.get_long_shares() / 100 - item.get_short_shares() / 100
        )  # inventory imbalance = long - short
        # print("imbalance", imbalance)
        # print("bp",self.trader.get_portfolio_summary().get_total_bp())
        # submit limit orders
        if order_size > 0:
            p_ask = round(
                (mid + (spread * (0.5 * (1 + symmetric_e) + asymmetric_e))), 2
            )
            p_bid = round(
                (mid - (spread * (0.5 * (1 + symmetric_e) - asymmetric_e))), 2
            )
            limit_buy = shift.Order(
                shift.Order.Type.LIMIT_BUY, ticker, order_size, p_bid
            )
            limit_sell = shift.Order(
                shift.Order.Type.LIMIT_SELL, ticker, order_size, p_ask
            )
    ...
    def get_states(self):
        ...
        diff_hedging_cost = (
            (hf_sp * abs(self.current_state_info[4])) * hedge_ratio
        ).tolist()
        # is bp at risk
        # identifier:
        ...
        return np.array(
            ident
            + self.current_state_info[6]
            + [self.current_state_info[4]]
            + [self.last_market_share]
            + self.current_state_info[1]
            + self.current_state_info[2]
            + diff_hedging_cost
            + [100 * self.current_state_info[3] / self.init_bp]
        )
    ...
    def step(self, actions):
        if self.init:
            self.init = False
            limit_buy = shift.Order(
                shift.Order.Type.LIMIT_BUY, self.symbol, 50, 100 - 0.02
            )
            limit_sell = shift.Order(
                shift.Order.Type.LIMIT_SELL, self.symbol, 50, 100 + 0.02
            )
        ...
        state_info = (
            self.compute_state_info()
        )  #:'curr_mp', 'ask_book', 'bid_book', 'remained_bp', 'current_inventory', spread, "his_prices", 'current_inventory_pnl', total_volume
        # print(state_info)
        ...
        # calculate reward: ###########################################################################################################################################
        recent_pl_change = (
            self.trader.get_portfolio_summary().get_total_realized_pl() - self.total_pl
        )
        # market share
        ...
        if state_info[8] == 0:
            ...
            current_market_share = (
                current_order_sizes / state_info[8]
            )  # changed to current agent' shares / market total shares, # actions[0]

        diff_market_share = abs(
            (current_market_share - self.target_market_share)
        )  # paper may has a typo：  - (self.last_market_share - self.target_market_share)
        change_inven_pnl = abs(state_info[7] - self.last_inventory_pnl)
        reward = (
            self.w * self.alpha * (recent_pl_change - (self.gamma * change_inven_pnl))
            - (1 - self.w) * diff_market_share
        )
        ...
        while self.trader.is_connected() and self.order_thread_alive:
            for i, order_id in enumerate(self.order_list):
                ...
                if (
                    ...
                    or order.status == shift.Order.Status.REJECTED
                    or order.status == shift.Order.Status.FILLED
                ):
            ...
            if len(self.order_list) >= self.max_order_list_length:
                for i, order_id in enumerate(
                    self.order_list[: int(self.max_order_list_length / 2)]
                ):
                    order = self.trader.get_order(order_id)
    ...
    def save_to_csv(self, epoch):  # TODO replace with logger
        ...
        try:
            self.df.to_csv(f"./iteration_info/{epoch}.csv")
            self.df = pd.DataFrame(columns=self._cols)
...
################Below is the ENV for MM with 4 actions, including Hedge ratio################################
...
#         #self.maxsize = 8                  ##############################important for action size#########################################
#         #now it's % instead of action order size
#         self.action_space= gym.spaces.Box(low = np.array([0.000005, -1, -1, 0]), high = np.array([0.0001, 1, 1, 0.5]), shape = (4,))
#         #self.action_space= gym.spaces.Box(low = np.array([0.000005, -1, -1, 0]), high = np.array([0.0001, 1, 1, 1]), shape = (4,))
...
#             self.generate_mm_trader(self.order_list, self.trader, self.symbol, order_size, parameters[1], parameters[2], parameters[3])
#         self.current_action_index = actions
#         #print(f"Action: {self.action_list[index]}, {self.symbol}")
...
#         #print("hedge",hedge_amount)
#         # submit market hedge orders
#         ask_book = trader.get_order_book(ticker, shift.OrderBookType.LOCAL_ASK)
#         bid_book = trader.get_order_book(ticker, shift.OrderBookType.LOCAL_BID)

#         if hedge_amount > 0:
#             left_hedge_amount = hedge_amount
#             for order in bid_book:
#                 if left_hedge_amount <= order.size:
#                     limit_sell = shift.Order(shift.Order.Type.LIMIT_SELL, ticker, left_hedge_amount, order.price)
#                     break
#                 else:
#                     limit_sell = shift.Order(shift.Order.Type.LIMIT_SELL, ticker, order.size, order.price)
#                     left_hedge_amount = left_hedge_amount - order.size
#                 order_list.append(limit_sell.id)
#                 trader.submit_order(limit_sell)
...
#         elif hedge_amount < 0:
#             left_hedge_amount = abs(hedge_amount)
#             for order in ask_book:
#                 if left_hedge_amount <= order.size:
#                     limit_buy = shift.Order(shift.Order.Type.LIMIT_BUY, ticker, left_hedge_amount, order.price)
#                     break
#                 else:
#                     limit_buy = shift.Order(shift.Order.Type.LIMIT_BUY, ticker, order.size, order.price)
#                     left_hedge_amount = left_hedge_amount - order.size
#                 order_list.append(limit_buy.id)
#                 trader.submit_order(limit_buy)
#             # market_buy = shift.Order(shift.Order.Type.MARKET_BUY, ticker, abs(hedge_amount))
