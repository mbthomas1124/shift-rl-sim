...
from Comp_rl_mm import SHIFT_env as Env
...
if __name__ == "__main__":
    ...
    for ticker in tickers:
        if ticker == "CS1": mm_index, lt_index, rang = 0, 0, range(num_proc)
        elif ticker == "CS2": mm_index, lt_index, rang = 1, 5, range(num_proc, num_proc+1)
        for i in range(num_proc):
            if agent_type[i] == "mm":
                trader_list.append(shift.Trader(f"marketmaker_rl_{mm_index+1}"))
                threads.append(Thread(target=test_ppo,args =(trader_list[len(trader_list)-1], f"mm{mm_index+1}", weights[i], agent_type[i], ticker, get_args())))     
                print(f"marketmaker_rl_{mm_index+1}")   
                mm_index += 1
