...
def test_ppo(trader, trader_id, identifiers, agent_type, agent_setup, ticker, args):
    ...
    try:
        ...
        print(sys.argv)
        if len(sys.argv) == 2:
            curr_run_num = int(sys.argv[1])
            # log
            base_path = "/home/shiftpub/Results_Simulation"
            log_path = os.path.join(base_path, "log")
            if curr_run_num == 1:
                last_checkpoint_path = os.path.join(log_path, f"checkpoint")
            else:
            ...
            if not os.path.exists(iteration_info_path):
...
if __name__ == "__main__":
    ...
    # event_path = os.path.join("/home/shiftpub/Results_Simulation/log", "event")
