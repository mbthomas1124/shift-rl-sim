...
from tianshou.utils.net.discrete import (
    Actor as discrete_Actor,
    Critic as discrete_Critic,
)
    ...
    try:
        ...
        #########################Liqudity Taker################################
            ...
            net = Net(
                ...
                device=args.device,
                softmax=True,
                norm_layer=torch.nn.InstanceNorm1d,  # norm_args=args.state_shape,
            )
            actor = discrete_Actor(net, args.action_shape, device=args.device).to(
                args.device
            )
            critic = discrete_Critic(net, device=args.device).to(args.device)
            actor_critic = ActorCritic(actor, critic)
            # orthogonal initialization
            for m in actor_critic.modules():
                if isinstance(m, torch.nn.Linear):
        ...
        train_collector = Collector(  # Async
            ...
            train_envs,
            VectorReplayBuffer(
                args.buffer_size, len(train_envs), exploration_noise=True
            ),  #
        )
        ...
        def save_best_fn(policy):
            ...
            torch.save(
                policy.state_dict(), os.path.join(log_path, f"policy{trader_id}.pth")
            )
        ...
        torch.save(
            policy.state_dict(),
            os.path.join(log_path, f"policy_ppo_trader{trader_id}.pth"),
        )
        ...
        torch.save(
            policy.state_dict(),
            os.path.join(log_path, f"policy_ppo_temp{trader_id}.pth"),
        )
...
if __name__ == "__main__":
    ...
    for ticker in tickers:
        if ticker == "CS1":
            mm_index, lt_index = 0, 0
        elif ticker == "CS2":
            mm_index, lt_index = 2, 10
    ...
    try:
        for i in range(len(trader_list)):  # len(tickers)*
            ...
            sleep(1)
            print(
                f"bp of {i+1}:", trader_list[i].get_portfolio_summary().get_total_bp()
            )
