...
def test_ppo(trader, trader_id, identifiers, agent_type, ticker, args=get_args()):
    ...
    try:
        ...
        # log
        base_path = "/home/shiftpub/Results_Simulation2"
        ...
        #     #global save_indicator, finish_saving
        #     print(save_indicator, finish_saving)
        #     ckpt_path = os.path.join(checkpoint_path, f"checkpoint_{trader_id}.pth")
        #     while finish_saving == False:
        #         if save_indicator == True:
        #             torch.save(
        #                 {"model": policy.state_dict(),
        #                     "optim": optim.state_dict(), }, ckpt_path
        #             )
        #             save_indicator = False
        #         print("saved checkpoint")
        #         sleep(1)       
        # saving_thread = Thread(target=checkpoint_thread)
        # saving_thread.start()
...
if __name__ == "__main__":
    ...
    try:
        ...
        crash_maker = shift.Trader("flash_crash_maker_02")
        crash_maker.disconnect()
        crash_maker.connect("/home/shiftpub/initiator.cfg", "password")
        crash_maker.sub_all_order_book()

        flash_crash_thread = Thread(target=flash_crash, args=(crash_maker, "sell", 3000, 10, 1, 285, 120, "CS2"))
